 ...
 * This program uses class CircleData, defined in CircleData.java.
 */
public class GrowingCircleAnimation extends Application {
    
    private CircleData[] circle; // holds the data for all 100 circles
    ...
    private void drawFrame(GraphicsContext g, int frameNumber, int width, int height) {
        ...
        if (circle == null) {  // create the array, if it doesn't exist
            circle = new CircleData[100];
            for (int i = 0; i < circle.length; i++) {
                circle[i] = new CircleData( 
        ...
        for (int i = 0; i < circle.length; i++) {  // draw the filled circles
            circle[i].radius++;
            circle[i].draw(g);
            if (Math.random() < 0.005 || circle[i].radius > 100) {
